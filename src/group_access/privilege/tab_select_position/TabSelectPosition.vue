...
export default defineComponent({
  ...
  methods: {
      ...
      switch (action) {
        ...
        case 'SHOW_POSITION_MODAL':
          ...
          selectPositionModal.getPositionDatas = async function (this: InstanceType<typeof SelectPositionModal>) {
            try {
              let positions = await PrivilegeService.position.getPositionsByWithoutPrivilegeId(self.router.params.id as any, {
    ...
    async getPositions() {
      try {
        let resData = await PrivilegeService.position.getsByPrivilegeId(this.router.params.id as any, {});
        return resData;
      } catch (ex) {
    ...
    async submit(position_id: number) {
      try {
        let resData = await PrivilegeService.position.join(position_id, this.router.params.id as any);
        this.baseMounted()
      } catch (ex) {
...
<template>
  <div class="row g-2 align-items-center">
    <div class="col">
      ...
      <span class="text-muted">You can add position to join this privilege</span>
    </div>
